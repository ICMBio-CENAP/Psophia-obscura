...
jags.data <- list(#y = y, block=block, nblock=length(unique(block)), nsite = length(y), nrep = 10, nyear = 4,
  #y = y, block=block, nblock=length(unique(block)), nsite = dim(y)[1], nrep = nrep, nyear = dim(y)[2],
  y = y, nsite = dim(y)[1], nrep = nrep, nyear = dim(y)[2], block=block, nblock=length(unique(block)),
  elevation=point.elevation, distEdge=distEdge, distWater=distWater, basalArea=basalArea, recovery=recovery)

# Initial values
zst <- apply(y, c(1, 2), max)	# Observed occurrence as inits for z
inits <- function(){ 
...
#out <- jags(data= jags.data, parameters.to.save=params, model.file=here("bin", "Dynocc_covariates.jags"), n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
