...
pobscura <- readRDS(here("data", "pobscura_40d.rds"))
names(pobscura)
...
# if using 30 days/year and 6-day occasion:
y <- array(c(unlist(pobscura[,2:15]),
             unlist(pobscura[,16:29]),
             unlist(pobscura[,30:43]),
             unlist(pobscura[,44:57]),
             unlist(pobscura[,58:71]) ), c(61, 14, 5))
str(y)
...
SiteCovs <- pobscura[,72:81] # if using 30 days/year and 6-day occasion
names(SiteCovs)
...
saveRDS(psophia_data_ijk, here("data", "psophia_data_ijk_40d.rds"))
