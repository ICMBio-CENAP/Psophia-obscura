...
model {
  ...
  for (i in 1:nsite){
    ...
    for (k in 2:nyear){
      logit(phi[i,k-1]) <- alpha.phi + beta.phi[1]*elevation[i] + beta.phi[2]*basalArea[i] + beta.phi[3]*recovery[i]
      logit(gamma[i,k-1]) <- alpha.gam + beta.gam[1]*elevation[i] + beta.gam[2]*basalArea[i] + beta.gam[3]*recovery[i]
   ...
   for (i in 1:nsite){
       for (k in 1:nyear){
         logit(p[i,k]) <-  alpha.p + beta.p*block[i] #+ eps[k]
...
saveRDS(out, here("results", "pobscura_mod_sel_3predictors.rds"))
...
par(mfrow=c(1,2))
hist(out$BUGSoutput$sims.list$beta.psi[,3], nclass=20, xlab="beta", main="")
hist(out$BUGSoutput$sims.list$beta.psi[,4], nclass=20, xlab="beta", main="")
dev.off()
