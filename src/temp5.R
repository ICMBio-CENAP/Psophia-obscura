...
model {
  ...
  # ecological model
  for (i in 1:nsite){
    logit(psi[i,1]) <-  alpha.psi + beta.psi[1]*distEdge[i] + beta.psi[2]*basalArea[i] + beta.psi[3]*recovery[i]
    z[i,1] ~ dbern(psi[i,1])
    
    for (k in 2:nyear){
      logit(phi[i,k-1]) <- alpha.phi + beta.phi[1]*distEdge[i] + beta.phi[2]*basalArea[i] + beta.phi[3]*recovery[i]
      logit(gamma[i,k-1]) <- alpha.gam + beta.gam[1]*distEdge[i] + beta.gam[2]*basalArea[i] + beta.gam[3]*recovery[i]
   ...
   #eval[i,k] <- dbin(psi[i,k], nrep[i,k])   	# Expected values
