...
model {
  ...
  # ecological model
  for (i in 1:nsite){
    logit(psi[i,1]) <-  alpha.psi + beta.psi[1]*block[i] + beta.psi[2]*elevation[i] + beta.psi[3]*distEdge[i] + beta.psi[4]*basalArea[i] + beta.psi[5]*recovery[i]
    z[i,1] ~ dbern(psi[i,1])
    
    for (k in 2:nyear){
      logit(phi[i,k-1]) <- alpha.phi + beta.phi[1]*block[i] + beta.phi[2]*elevation[i] + beta.phi[3]*distEdge[i] + beta.phi[4]*basalArea[i] + beta.phi[5]*recovery[i]
      logit(gamma[i,k-1]) <- alpha.gam + beta.gam[1]*block[i] + beta.gam[2]*elevation[i] + beta.gam[3]*distEdge[i] + beta.gam[4]*basalArea[i] + beta.gam[5]*recovery[i]
   ...
   for (i in 1:nsite){
       for (k in 1:nyear){
         logit(p[i,k]) <-  alpha.p + beta.p[1]*block[i] + beta.p[2]*elevation[i] + beta.phi[3]*distEdge[i] + beta.p[4]*basalArea[i] + beta.p[5]*recovery[i]
...
coef.function <- function(x) {
  coefs <- data.frame(x$BUGSoutput$summary[c("beta.psi[1]", "beta.psi[2]", "beta.psi[3]", "beta.psi[4]", "beta.psi[5]",
                                             "beta.phi[1]", "beta.phi[2]", "beta.phi[3]","beta.phi[4]","beta.phi[5]",
                                             "beta.gam[1]", "beta.gam[2]", "beta.gam[3]","beta.gam[4]","beta.gam[5]",
                                             "beta.p[1]", "beta.p[2]", "beta.p[3]", "beta.p[4]", "beta.p[5]"),])
  coefs <- tibble(predictor=c("block", "elevation", "edges", "basal.area", "recovery",
                              "block", "elevation", "edges", "basal.area", "recovery",
                              "block", "elevation", "edges", "basal.area", "recovery",
                              "block", "elevation", "edges", "basal.area", "recovery"),
...
predictor.effects.psi(out, log(pobscura$point.elevation), 2)
mtext("Elevation (m)", side=1, line=3)

predictor.effects.psi(out, pobscura$basal.area, 4)
