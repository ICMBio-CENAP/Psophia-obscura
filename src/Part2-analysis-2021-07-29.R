...
# ecological model
for (i in 1:nsite){
  logit(psi[i,1]) <- alpha.psi + beta.psi[1]*elevation[i] + beta.psi[2]*basalArea[i] + beta.psi[3]*distEdge[i] + beta.psi[4]*recovery[i]
  z[i,1] ~ dbern(psi[i,1])
...
for (i in 1:nsite){
  for (k in 1:nyear){
    logit(p[i,k]) <- alpha.p + beta.p*nrep[i,k] + eps[k]
...
coef.function <- function(x) {
  coefs <- data.frame(x$BUGSoutput$summary[c("beta.psi[1]", "beta.psi[2]", "beta.psi[3]", "beta.psi[4]", "beta.psi[5]",
                                             "beta.p"),])
  coefs <- tibble(predictor=c("elevation", "distWater", "basalArea", "distEdge", "recovery", "effort"),
