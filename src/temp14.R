...
min <- min(c(out$BUGSoutput$sims.list$u.B1, out$BUGSoutput$sims.list$u.B2))
max <- max(c(out$BUGSoutput$sims.list$u.B1, out$BUGSoutput$sims.list$u.B2))
par(mfrow=c(2,1))
hist(out$BUGSoutput$sims.list$u.B1, xlab="u.B1", main="", xlim=(c(min, max)) )
hist(out$BUGSoutput$sims.list$u.B2, xlab="u.B2", main="", xlim=(c(min, max)) )
#dev.off()
