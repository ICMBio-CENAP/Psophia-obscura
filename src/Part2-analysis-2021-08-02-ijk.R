...
# ecological model
for (i in 1:nsite){
  logit(psi[i,1]) <- alpha.psi[i] + beta.psi[1]*elevation[i] + beta.psi[2]*distEdge[i] + beta.psi[3]*basalArea[i] + beta.psi[4]*treeDensity[i] + beta.psi[5]*recovery[i]
  z[i,1] ~ dbern(psi[i,1])
...
inits <- function(){
  ...
  inits_d[inits_d > 0] <- 1
  list(z=as.matrix(cbind(inits_a, inits_b, inits_c, inits_d)))
  }
#inits()
