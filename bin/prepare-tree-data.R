...
# calculos abaixo baseados em: Mitchell (2015) Quantitative Analysis by the Point-Centered Quarter Method
...
# PAREI AQUI!!! O RESTO DO CODIGO É ANTIGO
#------------------------------------------------------------------------------
...
tree.dens.ha <- tree.dens*10000
hist(tree.dens.ha, main="Tree density near TEAM camera-stations, Gurupi Biological Reserve", xlab="Trees/ha")

#------------------------------------------------------------------------------

# COMANDOS ABAIXO FORAM MANTIDOS APENAS COMO EXEMPLOS PARA CALCULAR A AREA BASAL DE ARVORES MORTAS
# APAGAR DEPOIS DE AJUSTAR PARA FORMATO ACIMA

# distancia media arvores MORTAS
mean.dist.96m <- sum(p96[p96$morta=="y",]$dist.m)/nrow(p96[p96$morta=="y",])
...
(nrow(p96[p96$morta=="y",])/nrow(p96))
(nrow(p95[p95$morta=="y",])/nrow(p95))
(nrow(p33[p33$morta=="y",])/nrow(p33))
(nrow(p24[p24$morta=="y",])/nrow(p24))
(nrow(p23[p23$morta=="y",])/nrow(p23))
